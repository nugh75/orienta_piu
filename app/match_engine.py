...
def score_dimensional_similarity(school1: pd.Series, school2: pd.Series) -> Tuple[float, Dict]:
    ...
    max_dist = np.sqrt(5 * (6 ** 2))  # 5 dimensioni, range 1-7 = diff max 6
...
def score_complementarity(school1: pd.Series, school2: pd.Series, gap_threshold: float = 2.0) -> Tuple[float, Dict]:
    ...
    Args:
        gap_threshold: soglia per considerare una dimensione come "debole" (scala 1-7)
    ...
    for i, dim in enumerate(DIMENSIONS):
        gap1 = 7.0 - v1[i]  # Gap dal massimo in scala 1-7
        if gap1 >= gap_threshold:  # school1 è debole qui (gap >= 2 punti dal max)
            weaknesses.append(dim)
            if v2[i] >= 5.0:  # school2 è forte qui (>= 5/7)
                complements.append({
                    'dimension': dim,
                    'school1_score': v1[i],
                    'school2_score': v2[i],
...
def score_adjacency(school1: pd.Series, school2: pd.Series, margin: float = 1.0) -> Tuple[float, Dict]:
    ...
    Args:
        margin: margine di miglioramento desiderato in scala 1-7 (default 1 punto)
    ...
    if 0.3 <= diff <= 2.0:
        ...
        # Troppo avanti
        score = max(0, 50 - (diff - 2.0) * 20)
    else:
        # Indietro o uguale
        score = max(0, 50 + diff * 30)
...
def get_improvement_suggestions(
    ...
    for i, dim in enumerate(DIMENSIONS):
        if target_vector[i] < 4.0:  # Sotto la sufficienza (< 4/7)
            weak_dimensions.append({
                'dimension': dim,
                'label': DIMENSION_LABELS.get(dim, dim),
                'score': target_vector[i],
                'gap': 7.0 - target_vector[i]
            })
    ...
    for weakness in weak_dimensions:
        ...
        strong_peers = peers_df[peers_df[dim] >= 5.0].head(3)
...
def _build_strength_tags(df: pd.DataFrame) -> pd.Series:
    ...
    def tags_for_row(row: pd.Series) -> List[str]:
        ...
        if float(row.get("2_6_didattica_laboratoriale_score", 0) or 0) >= 5.0:
            tags.append("Laboratori")
        if float(row.get("mean_opportunita", 0) or 0) >= 5.0:
        ...
        if float(row.get("activities_count", 0) or 0) >= activities_threshold > 0:
            tags.append("Extra")
        if float(row.get("2_7_opzionali_sportive_score", 0) or 0) >= 5.0:
            tags.append("Sport")
        if float(row.get("2_7_opzionali_laboratoriali_espressive_score", 0) or 0) >= 5.0:
            tags.append("Arte/Musica")
        if float(row.get("2_5_azione_sistema_integrato_inclusione_fragilita_score", 0) or 0) >= 5.0:
