...
class TaskRunner {
  ...
  async init() {
    ...
    this.elements.btnKill.addEventListener("click", () =>
      this.stopCurrentTask(true),
    );
    this.elements.btnClear.addEventListener("click", () => this.clearOutput());
    this.elements.btnClearFinished.addEventListener("click", () =>
  ...
  selectCommand(cmd) {
    ...
    for (const info of varInfo) {
      ...
      // Se ci sono opzioni predefinite o dipendenze, usa un select
      if ((info.options && info.options.length > 0) || info.depends_on) {
      ...
      // Gestione dipendenze (es: PROVIDER -> MODEL)
      if (info.depends_on) {
        const depField = info.depends_on.field; // es: "PROVIDER"
        const optionsMap = info.depends_on.options_map; // { "ollama": [...], ... }

        // Salva riferimento per setup listener
        if (!cmd._dependencies) cmd._dependencies = [];
        cmd._dependencies.push({
          target: info.name, // es: "MODEL"
          source: depField, // es: "PROVIDER"
          map: optionsMap,
          defaultValue: info.options ? info.options[0] : "",
        });
      }
    ...
    // Setup listeners per dipendenze dopo aver creato tutti i campi
    if (cmd._dependencies) {
      for (const dep of cmd._dependencies) {
        const sourceInput = this.elements.paramsForm.querySelector(
          `[name="${dep.source}"]`,
        );
        const targetSelect = this.elements.paramsForm.querySelector(
          `select[name="${dep.target}"]`,
        );

        if (sourceInput && targetSelect) {
          const updateTarget = () => {
            const sourceVal = sourceInput.value;
            const validOptions = dep.map[sourceVal] || dep.map["auto"] || [];
            ...
            emptyOpt.value = "";
            emptyOpt.textContent = "-- Auto / Default --";
            ...
            // Aggiungi custom se permesso (verifica info originale se possibile, o assumi true per target come MODEL)
            // Per ora aggiungiamo sempre custom per MODEL/ANALYST etc se era previsto
            // (Recuperiamo info originale dal cmd.variable_info)
            const targetInfo = cmd.variable_info.find(
              (v) => v.name === dep.target,
            );
            if (targetInfo && targetInfo.allow_custom) {
              const customOpt = document.createElement("option");
              customOpt.value = "__custom__";
              customOpt.textContent = "✏️ Altro (personalizzato)...";
            ...
            // Reset input custom se presente
            const customInput = document.getElementById(
              `param-${dep.target}-custom`,
            );
            if (customInput) {
              customInput.classList.add("hidden");
              customInput.value = "";
  ...
  async startSelectedCommand() {
    ...
    const inputs = this.elements.paramsForm.querySelectorAll(
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.appendOutput(`Errore: ${data.error || response.statusText}`, "error");
        }
      } catch (e) {
        console.error("Failed to parse response:", e);
        this.appendOutput(`Errore parsing risposta: ${e.message}`, "error");
        if (responseText) {
            this.appendOutput(`Contenuto risposta: ${responseText.substring(0, 100)}...`, "error");
        }
      }
    } catch (error) {
      console.error("Failed to start task:", error);
      this.appendOutput(`Errore di connessione: ${error.message}`, "error");
    }
  }
  ...
  connectTaskStream(taskId) {
    ...
    this.taskStream = new EventSource(
      `${this.baseUrl}/sse/tasks/${taskId}/stream`,
    );
      ...
      try {
        ...
        if (data.type === "output") {
          ...
          if (task) {
            ...
            this.appendOutput("");
            this.appendOutput(
              `>>> Task ${task.status} (exit: ${task.exit_code})`,
              status,
            );
          }
          this.taskStream.close();
  ...
  connectGlobalEvents() {
    ...
    this.eventSource.onerror = (e) => {
      ...
      console.log(
        `Riconnessione SSE in ${delay / 1000}s (tentativo ${this.reconnectAttempts})...`,
      );
      setTimeout(() => {
        this.connectGlobalEvents();
      }, delay);
  ...
  async stopCurrentTask(force = false) {
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/api/tasks/${this.currentTaskId}/stop`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async clearFinishedTasks() {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const count = data.archived_count !== undefined ? data.archived_count : data.removed;
        this.appendOutput(
          `>>> Archiviati ${count} task completati`,
          "info",
        );
        this.loadTasks();
      }
  ...
  appendOutput(line, className = "") {
    ...
    // Auto-scroll
    this.elements.outputContainer.scrollTop =
      this.elements.outputContainer.scrollHeight;
  }
